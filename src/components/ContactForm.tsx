...
import { X, User, Phone, MessageCircle, CheckCircle, Sparkles, Download } from "lucide-react";
...
const ContactForm: React.FC<ContactFormProps> = ({ isOpen, onClose, type }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLeadToAppsScript({ name: formData.name.trim(), number: formData.number.trim(), source: isBrochure ? 'brochure' : 'enquire' });
  ...
  const title = isBrochure ? 'Download Brochure' : 'Get More Details';
  const subtitle = isBrochure ? 'Fill in your details to download the brochure' : 'Fill in your details to receive personalized information';
  const buttonText = isBrochure ? 'Download Brochure' : 'Submit Details';
  const successTitle = isBrochure ? 'Brochure Downloaded!' : 'Thank You!';
  const successMessage = isBrochure ? 'Your brochure download request has been received' : 'Your details have been submitted successfully';
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              >
                {isSubmitting ? (
                  ...
                  <div className="flex items-center gap-2">
                    {isBrochure ? <Download className="w-4 h-4" /> : <MessageCircle className="w-4 h-4" />}
            ...
            <div className="p-6 flex flex-col items-center justify-center h-full text-center space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-gray-600 leading-relaxed">
                  {isBrochure ? (
                    <>
                      Your brochure download request has been received. Our team will contact you soon at 
                      <span className="font-semibold text-primary"> {formData.number}</span> to provide the brochure.
                    </>
                  ) : (
                    <>
              ...
              <div className="bg-gradient-to-r from-primary/10 to-gold/10 rounded-xl p-4 w-full">
                ...
                <ul className="text-xs text-gray-600 space-y-1">
                  ...
                  <li>• {isBrochure ? 'You\'ll receive the brochure via WhatsApp/Email' : 'You\'ll receive exclusive project details'}</li>
